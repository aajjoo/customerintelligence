...
**Projekte-Bereich mit Jira/Confluence (23.07.2026, Konzept Kap. 5):** Eigener Navigationspunkt /projekte mit allen laufenden Projekten der sichtbaren Kunden. Je Projekt: Ticket-Entwicklung aus Jira (offen/in Arbeit/erledigt, angelegt vs. gelöst über 8 Wochen), verbrauchte vs. budgetierte Stunden (aus Jira-Worklogs oder manuell, mit Fortschrittsbalken), DB-Ziel und eine Claude-Einschätzung: Ampel, Lage, konkrete Probleme (aus Jira-Tickets/-Kommentaren und Confluence-Auszügen) und ob das DB-Ziel noch erreichbar scheint – jede Aussage mit Quelle (Kernregel 1). Anlage auf zwei Wegen: manuell mit Ziel-URLs (Jira-Key, Jira-/Confluence-URL, Budget, DB-Ziel) oder Import der laufenden Projekte direkt aus Jira (Konzept 5.1: führendes System, keine Doppelpflege). Jira/Confluence sind env-gated (JIRA_BASE_URL, JIRA_EMAIL, JIRA_API_TOKEN – Jira Cloud REST v3); ohne Konfiguration funktioniert alles Manuelle weiter, die Einschätzung nutzt dann Stunden-/Budgetdaten. Mail-Inhalte als zusätzliche Signalquelle folgen, sobald ein Postfach-Konnektor (Google Workspace) bereitsteht. Außerdem: Signalvolumen-Chart der Kundenseite jetzt wochenweise (8 Wochen statt 6 Monate – bei einem jungen Radar aussagekräftiger); Aufgaben haben eine eigene kundenübergreifende Seite (/aufgaben); Team-Zuweisung je Kunde in der Verwaltung.
